...
## Primary navigation
- The main authenticated experience uses a custom five-item bottom bar rather than Expo Router's stock tabs because the center Agendar action is a branded floating circular button. Home is the daily dashboard route; Perfil is the existing booked-classes/account route, so the two are distinct.
  ...
  **How to apply:** Keep the bar mounted from the root layout only for top-level authenticated routes; preserve a 44pt+ touch target, safe-area padding, and a visible label under every icon. Keep the splash redirect pointed at Home, not Profile.
